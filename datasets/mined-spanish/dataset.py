...
def preprocess_conala_dataset(train_file, test_file, grammar_file, src_freq=3, code_freq=3):
    ...
    pickle.dump(train_examples, open('data/mined-spanish/train.var_str_sep.bin', 'wb'))
    pickle.dump(full_train_examples, open('data/mined-spanish/train.var_str_sep.full.bin', 'wb'))
    # pickle.dump(dev_examples, open('data/mined-spanish/dev.var_str_sep.bin', 'wb'))
    pickle.dump(train_examples, open('data/mined-spanish/dev.var_str_sep.bin', 'wb'))
    pickle.dump(test_examples, open('data/mined-spanish/test.var_str_sep.bin', 'wb'))
    pickle.dump(vocab, open('data/mined-spanish/vocab.var_str_sep.new_dev.src_freq%d.code_freq%d.bin' % (src_freq, code_freq), 'wb'))
...
def preprocess_dataset(file_path, transition_system, name='train'):
    ...
    for i, example_json in enumerate(dataset):
        ...
        try:
            ...
            print("Question error on: " + str(example_json['question_id']))
            continue
        ...
        # if code_from_hyp != canonical_code:
        #     print(code_from_hyp)
